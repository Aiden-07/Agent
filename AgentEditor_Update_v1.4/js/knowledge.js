...
function renderCreateKbFileList() {
    ...
    createKbFiles.forEach((file, index) => {
        ...
        const defaultParserValue = defaultParserEl ? defaultParserEl.value : 'general';
        const defaultParserText = defaultParserValue === 'process' ? '工艺图解析' : '通用解析';
        ...
        row.innerHTML = `
            ...
            <div class="w-24 text-center text-xs text-gray-500 border-l border-r border-gray-100 px-2 truncate">${defaultParserText}</div>
...
function resetCreateKbForm() {
    ...
    const defaultParserEl = document.getElementById('create-kb-default-parser');
    if (defaultParserEl) defaultParserEl.value = 'general';
...
function submitCreateKb() {
    ...
    const defaultParser = defaultParserEl ? defaultParserEl.value : 'general';
    ...
    const newKb = {
        ...
        defaultParser: defaultParser, // Store default parser
        chunkSize: 500,
        retrievalMode: null,
...
let currentParserType = 'general';

function updateParserModalUI() {
    const generalBtn = document.getElementById('parser-radio-general');
    const processBtn = document.getElementById('parser-radio-process');
    const generalIcon = document.getElementById('parser-radio-icon-general');
    const processIcon = document.getElementById('parser-radio-icon-process');

    const isGeneral = currentParserType === 'general';
    if (generalBtn) generalBtn.setAttribute('aria-checked', isGeneral ? 'true' : 'false');
    if (processBtn) processBtn.setAttribute('aria-checked', isGeneral ? 'false' : 'true');

    if (generalIcon) {
        generalIcon.className = isGeneral ? 'fa-solid fa-circle-check text-blue-600' : 'fa-regular fa-circle text-gray-400'
    }
    if (processIcon) {
        processIcon.className = isGeneral ? 'fa-regular fa-circle text-gray-400' : 'fa-solid fa-circle-check text-blue-600'
    }
}

function selectParserType(type) {
    if (type !== 'general' && type !== 'process') return;
    currentParserType = type;
    updateParserModalUI();
}
window.selectParserType = selectParserType;

function openParserSelectModal() {
    const modal = document.getElementById('parser-select-modal');
    if (!modal) return;

    const targetIds = Array.isArray(window.batchParserTargetDocIds) ? window.batchParserTargetDocIds : [];
    if (Array.isArray(mockDocs) && targetIds.length === 1) {
        const doc = mockDocs.find(d => d.id === targetIds[0]);
        if (doc && doc.parserName === '工艺图解析器') {
            currentParserType = 'process';
        } else {
            currentParserType = 'general';
        }
    } else {
        currentParserType = 'general';
    }
...
window.openParserSelectModal = openParserSelectModal;

function closeParserSelectModal() {
    const modal = document.getElementById('parser-select-modal');
    if (!modal) return;
    modal.classList.add('hidden');
}
window.closeParserSelectModal = closeParserSelectModal;

function confirmParserSelect() {
    const parserName = currentParserType === 'process' ? '工艺图解析器' : '通用解析器';
    const targetIds = Array.isArray(window.batchParserTargetDocIds) ? window.batchParserTargetDocIds : [];
    if (Array.isArray(mockDocs) && targetIds.length > 0) {
        const idSet = new Set(targetIds);
        mockDocs.forEach(function (doc) {
            if (idSet.has(doc.id)) {
                doc.parserName = parserName;
            }
        });
        renderDocList();
    ...
    closeParserSelectModal();
}
window.confirmParserSelect = confirmParserSelect;
    ...
    visibleDocs.forEach(doc => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-6 py-4 text-gray-500">
                ${(function() {
                    const settingName = doc.sliceSettingName;
                    const isConfigured = settingName && settingName !== '未设置' && settingName !== '设置';
                    const displayText = isConfigured ? settingName : '未设置';
                    const btnClass = isConfigured 
                        ? 'text-blue-600 font-medium hover:text-blue-700 max-w-[120px] truncate block text-left' 
                        : 'text-[#999] font-normal hover:text-blue-600 max-w-[120px] truncate block text-left';
                    
                    const tooltip = displayText && displayText.length > 8 ? displayText : '打开切片设置';
                    
                    return `<button type="button" class="${btnClass} hover:underline transition-colors" onclick="event.stopPropagation(); openBatchSliceSettingsPageSingle('${doc.id}');" aria-label="打开切片设置" title="${tooltip}">
                        ${displayText}
                    </button>`;
                })()}
            </td>
            <td class="px-6 py-4 text-gray-500">
                ${(function() {
                    const parserName = doc.parserName;
                    const isConfigured = parserName && parserName !== '未设置' && parserName !== '设置' && parserName !== '待配置';
                    const displayText = isConfigured ? parserName : '未设置';
                    const btnClass = isConfigured 
                        ? 'text-blue-600 font-medium hover:text-blue-700 max-w-[120px] truncate block text-left' 
                        : 'text-[#999] font-normal hover:text-blue-600 max-w-[120px] truncate block text-left';
                    
                    const tooltip = displayText && displayText.length > 8 ? displayText : '打开解析器设置';

                    return `<button type="button" class="${btnClass} hover:underline transition-colors" onclick="event.stopPropagation(); window.batchParserTargetDocIds = ['${doc.id}']; openParserSelectModal();" aria-label="打开解析器设置" title="${tooltip}">
                        ${displayText}
                    </button>`;
                })()}
            </td>
...
function saveBatchSliceSettings() {
    ...
    const strategyName = currentSliceStrategy === 'special' ? '模版切片' : '通用切片';
    const targetIds = (function () {
    ...
    setTimeout(() => {
        if (Array.isArray(mockDocs) && targetIds.length > 0) {
            ...
            mockDocs.forEach(function (doc) {
                if (idSet.has(doc.id)) {
                    doc.sliceSettingName = strategyName;
                }
            });
            renderDocList();
        }
...
function openBatchSliceSettingsPageSingle(docId) {
    // Validate parser configuration for single doc
    const doc = window.getDocById(docId);
    if (doc) {
        if (!doc.parserName || doc.parserName === '未设置' || doc.parserName === '待配置') {
             const toast = document.createElement('div');
             toast.className = 'fixed top-4 left-1/2 transform -translate-x-1/2 bg-orange-500 text-white px-4 py-2 rounded-lg shadow-lg z-[60] flex items-center gap-2 animate-fade-in-down';
    ...
    if (typeof window.openBatchSliceSettingsPage === 'function') {
        window.batchSliceTargetDocIds = [docId];
