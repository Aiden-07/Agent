...
function renderModelTable() {
    ...
    if (pageData.length === 0) {
        ...
        pageData.forEach(model => {
            ...
            tr.innerHTML = `
                <td class="px-6 py-4 min-w-0"><span class="dt-cell-ellipsis font-medium" title="${esc(model.name)}">${esc(model.name)}</span></td>
                <td class="px-6 py-4 whitespace-nowrap"><span class="dt-cell-ellipsis" title="${esc(model.type)}">${esc(model.type)}</span></td>
                <td class="px-6 py-4 min-w-0"><div class="flex flex-nowrap gap-1 overflow-hidden">${capsHtml}</div></td>
    ...
    if (window.syncDataTable) window.syncDataTable('model-data-table', { storageKey: 'dt-colwidths-models' });
}
