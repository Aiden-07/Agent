...
function renderMcpList(list) {
    ...
    if (list.length === 0) {
        ...
        if (window.syncDataTable) window.syncDataTable('mcp-data-table', { storageKey: 'dt-colwidths-mcp' });
        return;
    }
    ...
    list.forEach(item => {
        ...
        const nameLine = escapeHtml(item.name);
        const titleName = escapeHtml(`${item.name || ''} (${item.id || ''})`);

        tr.innerHTML = `
            <td class="px-6 py-4 min-w-0">
                <button type="button" class="text-left w-full min-w-0 text-blue-600 hover:text-blue-800 font-medium dt-cell-ellipsis" title="${titleName}" onclick="openMcpDetail('${item.id}')">${nameLine}</button>
            </td>
            <td class="px-6 py-4 min-w-0 text-gray-600"><span class="dt-cell-ellipsis" title="${escapeHtml(item.description || '-')}">${escapeHtml(item.description || '-')}</span></td>
            <td class="px-6 py-4 whitespace-nowrap">${formatStatusPill(item.enabled)}</td>
            <td class="px-6 py-4 text-gray-600 whitespace-nowrap text-xs">${escapeHtml(item.createdAt || '-')}</td>
            <td class="px-6 py-4 text-gray-600 whitespace-nowrap text-xs">${escapeHtml(item.updatedAt || '-')}</td>
            <td class="px-6 py-4 text-right whitespace-nowrap min-w-[200px]">
                <div class="inline-flex items-center gap-3 text-sm justify-end">
