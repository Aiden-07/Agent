...
function renderCreateKbPreviewSlices() {
    ...
    if (createKbPreviewSlices.length > renderLimit) {
        ...
        moreHint.textContent = `还有 ${createKbPreviewSlices.length - renderLimit} 个切片未显示...`;
...
function updateSliceStrategyUI() {
    ...
    const specialTypesGrid = document.getElementById('create-kb-special-types-grid');
    const specialTypes = ['word', 'pdf', 'excel', 'ppt', 'image', 'text', 'invoice'];
    ...
    if (currentSliceStrategy === 'general') {
        ...
        // Hide special type grid
        if (specialTypesGrid) {
            specialTypesGrid.classList.add('hidden');
        }
        ...
        // Show special type grid
        if (specialTypesGrid) {
            specialTypesGrid.classList.remove('hidden');
        }
